...
#========================================================
def Paradigm(trials,training=False):
    terminate = False
    ...
    if training:
        mrkstream_out.push_sample(pylsl.vectorstr(['startTraining']))
    else:
        mrkstream_out.push_sample(pylsl.vectorstr(['startTesting']))
    ...
    # End of paradigm
    if training:
        # send marker to train the classifier
        mrkstream_out.push_sample(pylsl.vectorstr(['trainClassifier'])) 
        #mrkstream_out.push_sample(pylsl.vectorstr(['die'])) # terminate paradigm

    else:
        mrkstream_out.push_sample(pylsl.vectorstr(['endTesting']))
...
if __name__ == "__main__":
    ...
    trials = ['blink','noblink'] *1
    # repeat the trials 4 times
    # shuffle them
    random.shuffle(trials)
    Paradigm(trials,training=True)
    
    # Run through paradigm
    trials = ['blink','noblink'] * 1
    Paradigm(trials,training=False)